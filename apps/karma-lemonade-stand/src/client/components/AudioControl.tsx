...
export const AudioControl: React.FC<AudioControlProps> = ({ className = '' }) => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('audioMuteChanged', handleMuteChange as EventListener);

    return () => {
      window.removeEventListener('audioMuteChanged', handleMuteChange as EventListener);
    };
  }, [audioManager]);
  ...
  return (
    <button
      ...
      className={`audio-control ${className}`}
